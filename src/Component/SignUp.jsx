...
const SignUp = () => {
  ...
  useEffect(() => {
    console.log("User name stored:", localStorage.getItem("name")); // Check stored name
  }, []);
      ...
      <div className="w-1/2 flex flex-col justify-start items-center bg-white p-10 border-r max-h-full overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="w-full">
          <input
            ...
            className="w-full p-2 border rounded-md mb-3 focus:outline-none focus:ring-2 focus:ring-blue-800 shadow-sm text-sm"
          />
          <input
            type="email"
            placeholder="Email"
          ...
          <button type="submit" className="w-full p-2 bg-blue-800 text-white font-medium rounded-md hover:bg-blue-800 transition-all shadow-md text-sm">
            Sign Up
          </button>
        </form>

        <p className="mt-4 text-gray-600 text-sm">
          Already have an account? <Link to="/signin" className="text-blue-800 hover:underline">Sign In</Link>
        </p>
      </div>
